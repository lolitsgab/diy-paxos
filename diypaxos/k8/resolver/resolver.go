...
var (
	...
	allowUnusedImages = flag.Bool("allow_unused_images", true, "Allow images that don't appear in the JSON. This is useful when generating multiple SKUs of a k8s_object, only some of which use a particular image.")
	stampInfoFile     utils.ArrayStringFlags
	imgSpecs          utils.ArrayStringFlags
	loadDaemon        = flag.Bool("load_daemon", true, "Loads image into daemon and resolves images by tag, not digest.")
)
